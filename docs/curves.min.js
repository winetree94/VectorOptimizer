...
function rdpReduce(pts, error) {
    ...
    keepIndex.forEach(function (i) { return res.push(pts[i]); });
    return res;
}
...
function rdpRecursive(pts, error, first, last, keepIndex) {
    ...
    var a = pts[first];
    var b = pts[last];
    var abDist = a.distance(b);
    var aCrossB = a.x * b.y - b.x * a.y;
    var maxDist = error;
    var split = 0;
    for (var i = first + 1; i < last - 1; i++) {
        var p = pts[i];
        var pDist = perpendicularDistance(a, b, abDist, aCrossB, p);
        if (pDist > maxDist) {
            maxDist = pDist;
            split = i;
        }
